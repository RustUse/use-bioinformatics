...
FOCUSED_CRATES := use-sequence use-residue use-alphabet use-nucleotide use-amino-acid use-genomic-range use-annotation use-alignment use-sequence-id use-feature use-motif
...
help:
	@printf "%s\n" \
		...
		"publish-dry-run-facade         Dry-run publish use-bioinformatics after crates.io propagation" \
...
publish-dry-run-facade:
	cargo publish --dry-run --allow-dirty -p use-bioinformatics
